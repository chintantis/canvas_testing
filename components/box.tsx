...
const GoldPlanCard: React.FC<GoldPlanCardProps> = ({ title, price, planType, description, features }) => {
    const right = (
        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="9" style={{ background: '#f3f3ff', borderRadius: 50, marginRight: 10, height: 15, width: 15, marginLeft: 5, padding: 6 }} viewBox="0 0 12 11" fill="none">
    ...
    return (
        <Grid item lg={6} >
            <Box height={620} width={440} sx={{ background: 'white', boxShadow: '0px 0px 3px -1px #e4e1e1', borderRadius: 4, marginBottom: 9 }}>
                <Typography textAlign={'center'} color={'#492DAC'} fontSize={20} fontWeight={600} pt={4}>{title}</Typography>
                <Medium_text>{price}</Medium_text>
                <Typography textAlign={'center'} color={'#474747'} fontWeight={600}>
                    {planType}
                </Typography>
                <Cardtext textAlign={'center'} pt={2}>{description}</Cardtext>
                <Box>
                    ...
                    <Box>
                        <button className='button' style={{ margin: '31px 34px', padding: '10px 138px', whiteSpace: 'nowrap' }}>GET STARTED</button>
                    </Box>
                </Box>
            </Box>
        </Grid>
